...
class DocumentImportWizardLine(models.TransientModel):
    ...
    def _get_cfop_warning(self):
        ...
        if declared in ("1", "5") and not same_state:
            return _(
                "XML CFOP %(cfop)s is intrastate but issuer (%(issuer)s) and "
                "company (%(company)s) are in different states."
            ) % {
                "cfop": self.cfop_xml,
        ...
        if declared in ("2", "6") and same_state:
            return _(
                "XML CFOP %(cfop)s is interstate but issuer and company are "
                "both in %(company)s."
            ) % {"cfop": self.cfop_xml, "company": company_state.code}
        return False
