...
class InstallmentRenegotiationWizard(models.TransientModel):
    ...
    def _compute_totals(self):
        for wizard in self:
            ...
            wizard.original_total = sum(abs(l.amount_currency) for l in payment_lines)
    ...
    def _validate_renegotiation(self):
        ...
        if self.move_id.state != "posted":
            raise UserError(_("The invoice must be posted to renegotiate installments."))
    ...
    def action_apply(self):
        ...
        move_sudo = move.with_context(**ctx).sudo()
