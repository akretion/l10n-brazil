...
class TestInstallmentRenegotiation(TransactionCase):
    ...
    def test_action_opens_wizard(self):
        ...
        result = invoice.with_user(self.account_manager).action_renegotiate_installments()

        self.assertEqual(result["type"], "ir.actions.act_window")
        self.assertEqual(result["res_model"], "account.installment.renegotiation.wizard")
    ...
    def test_wizard_allows_date_change(self):
        ...
        wizard = self.env["account.installment.renegotiation.wizard"].with_user(
        ...
        original_dates = wizard.line_ids.mapped("date_maturity")
    ...
    def test_wizard_allows_amount_redistribution(self):
        ...
        new_total = sum(abs(l.amount_currency) for l in payment_term_lines)
    ...
    def test_add_installment(self):
        ...
        # Add new line
        new_line = self.env["account.installment.renegotiation.wizard.line"].create(
            {
                "wizard_id": wizard.id,
    ...
    def test_remove_installment(self):
        ...
        original_count = len(wizard.line_ids)
        self.assertGreaterEqual(original_count, 2, "Need at least 2 lines to test removal")
